...
from nanochat.common import compute_init, compute_cleanup, print0, get_base_dir, autodetect_device_type, get_peak_flops
from nanochat.experiment_logger import init_experiment_logger
...
# Logging
parser.add_argument("--run", type=str, default="dummy", help="experiment run name ('dummy' disables logging)")
parser.add_argument("--logger", type=str, default="auto", choices=["auto", "wandb", "swanlab", "none"], help="experiment logger backend")
# Runtime
...
if device_type == "cuda":
    accel_device_name = torch.cuda.get_device_name(0)
    peak_flops_per_device = get_peak_flops(accel_device_name)
    print0(f"GPU: {accel_device_name} | Peak FLOPS (BF16): {peak_flops_per_device:.2e}")
elif device_type == "npu":
    accel_device_name = torch.npu.get_device_name(0)
    peak_flops_per_device = get_peak_flops(accel_device_name)
    print0(f"NPU: {accel_device_name} | Peak FLOPS (BF16): {peak_flops_per_device:.2e}")
else:
    peak_flops_per_device = float('inf')  # MFU not meaningful for CPU/MPS

exp_logger, _logger_backend = init_experiment_logger(
    run_name=args.run,
    config=user_config,
    master_process=master_process,
    logger_choice=args.logger,
    wandb_project="nanochat-sft",
    swanlab_project="nanochat-sft",
    print_fn=print0 if master_process else None,
)
...
while True:
    ...
    promised_flops_per_sec = peak_flops_per_device * ddp_world_size
